...
public:
	...
	/*
	î{ó¶ÇÃï™ÇæÇØÉfÅ[É^ÇïKóvÇ∆Ç∑ÇÈ
...
// èc é©óRî‰ó¶èkè¨
// threadâªÇ∑ÇÈÇ∆ÇµÇΩÇÁÅAbodyÇÃtargetRatioÇÃloopÇï™äÑÇ©ÅBÅB
// targetRatioÇ™2ÇÃèÍçáÇ…ì‡ë§ÇÃbodyÇÃÉãÅ[ÉvÇ™ë∂ç›ÇµÇ»Ç¢ÇÃÇ≈ï™äÑèoóàÇ»Ç¢ÅBÇªÇÃèÍçáÇÕsrcRatioÇ™è¨Ç≥Ç¢èÍçáÇÕäOë§ÇÃÉãÅ[ÉvÇ™ï™äÑèoóàÇÈÇÃÇ≈ñ‚ëËñ≥Ç¢ÅBëÂÇ´Ç¢èÍçáÇÕèkè¨î‰ó¶é©ëÃÇ™ëÂÇ´Ç¢ÇÃÇ≈
void AveragingReducer::Process_RatioAny(ILineAveragingReducer& lineReducer, uint16_t part)
{
	...
	if (parts > 1 && blockCount == 1 && bodyCount >= 4) {
		...
		const uint16_t bodyEnd = (part == parts-1) ? bodyCount : (bodyStart + partLen);
		{
			...
			if (bodyStart == 0) {
				// head + body
				lineReducer.fillRead(pSrc, tmpBuffBody);
				OffsetPtr(pSrc, params.srcLineOffsetBytes);
				for (uint16_t i=0; i<endBodyCnt; ++i) {
					lineReducer.addRead(pSrc, tmpBuffBody);
					OffsetPtr(pSrc, params.srcLineOffsetBytes);
				}
				// srcRatio data straddles targetRatio's tail and next head
			...
			// body
			{
				...
				for (uint16_t i=bodyStart; i<bodyEnd; ++i) {
					...
					if (innerBodyCnt) {
						lineReducer.fillRead(pSrc, tmpBuffBody);
						OffsetPtr(pSrc, params.srcLineOffsetBytes);
						for (uint16_t j=0; j<innerBodyCnt-1u; ++j) {
							lineReducer.addRead(pSrc, tmpBuffBody);
							OffsetPtr(pSrc, params.srcLineOffsetBytes);
						}
						// tail and next head
